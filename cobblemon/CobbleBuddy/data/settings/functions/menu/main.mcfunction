...
execute if score pokeball_changer enabled matches 1 run tellraw @s {"text":"[Pokeball Changer]","bold":false,"color":"red","clickEvent":{"action":"run_command","value":"/trigger settings set -2"}}
...
execute if score fossil_locator enabled matches 1 run tellraw @s {"text":"[Fossil Locator]","bold":false,"color":"gray","clickEvent":{"action":"run_command","value":"/trigger settings set -6"}}
