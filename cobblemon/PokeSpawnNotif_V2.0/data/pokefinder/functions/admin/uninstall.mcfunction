...
tag @a remove pokespawner_admin
tag @a remove found
tag @a remove match
...
# Clear data storage
data remove storage pokespawner:temp CurrentPokemon
data remove storage pokespawner:data PokemonList
...
tellraw @a ["",{"text":"Run this command to disable: ","color":"gray"},{"text":"/datapack disable \"file/PokeSpawnNotif_V1.0.zip\"","color":"white","clickEvent":{"action":"suggest_command","value":"/datapack disable \"file/PokeSpawnNotif_V1.0.zip\""}}]