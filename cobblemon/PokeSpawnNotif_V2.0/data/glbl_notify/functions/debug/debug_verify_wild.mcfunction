...
execute if score #debug_cooldown gn_timer matches 0 run tellraw @a[tag=admin] ["DEBUG: Shiny Check: ",{"score":{"name":"#is_shiny","objective":"pokemon.temp"}}]
execute if score #debug_cooldown gn_timer matches 0 run tellraw @a[tag=admin] ["DEBUG: Legendary Check: ",{"score":{"name":"#is_legendary","objective":"pokemon.temp"}}]
