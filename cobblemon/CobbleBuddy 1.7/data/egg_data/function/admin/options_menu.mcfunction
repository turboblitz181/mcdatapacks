...
execute if score ed_prc enabled matches 1 run tellraw @s ["",{"text":"[✓] ","color":"green","bold":true},{"text":"Allow players to scan chests they look at","color":"white","clickEvent":{"action":"run_command","value":"/trigger ed_admin set 1"},"hoverEvent":{"action":"show_text","contents":"Click to disable"}}]
execute if score ed_prc enabled matches 0 run tellraw @s ["",{"text":"[✗] ","color":"red","bold":true},{"text":"Allow players to scan chests they look at","color":"gray","clickEvent":{"action":"run_command","value":"/trigger ed_admin set 1"},"hoverEvent":{"action":"show_text","contents":"Click to enable"}}]
