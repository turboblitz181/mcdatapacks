...
$execute if entity @a[tag=egg_data_show,scores={ed_shiny=1}] if data block ~ ~ ~ {Items:[{Slot:$(slot_id)b,id:"cobbreeding:pokemon_egg"}]} unless data block ~ ~ ~ Items[{Slot:$(slot_id)b,}].components.minecraft:custom_data.sh_set if score ed_shiny ed_success matches 0 run item modify block ~ ~ ~ container.$(slot_id) egg_data:none_lore 
$execute if entity @a[tag=egg_data_show,scores={ed_ivs=1}] if data block ~ ~ ~ {Items:[{Slot:$(slot_id)b,id:"cobbreeding:pokemon_egg"}]} unless data block ~ ~ ~ Items[{Slot:$(slot_id)b,}].components.minecraft:custom_data.iv_set run item modify block ~ ~ ~ container.$(slot_id) egg_data:ivs_lore
...
execute if score chest_loop ed_index matches ..26 run function egg_data:get_data_chest with storage cobblebuddy:ed_temp_data