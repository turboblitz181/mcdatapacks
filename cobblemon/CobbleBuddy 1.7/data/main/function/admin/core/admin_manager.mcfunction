...
tellraw @s ["\n",{"text":"Current admins:","color":"yellow"}]

# List ALL admins to the player who opened the menu (without ID and individual revoke)
execute at @s as @a[tag=admin] run tellraw @p[tag=admin] ["",{"text":"• ","color":"gray"},{"selector":"@s","color":"aqua"}]

# New general Revoke button
tellraw @s ["\n",{"text":"[Revoke Admin Rights]","color":"red","hoverEvent":{"action":"show_text","contents":"Click to open the admin revocation menu"},"clickEvent":{"action":"run_command","value":"/trigger admin set 81"}}]
